...
const AdminDashboard = () => {
  ...
  if (statsLoading) {
    return (
      <DashboardLayout title="Admin Dashboard" subtitle="Loading analytics and details...">
        <div style={{ padding: '24px 0' }}>
          <LoadingState type="skeleton-cards" cardsCount={4} />
          <div style={{ marginTop: '32px' }}>
            <LoadingState type="spinner" message="Preparing maps and chart layouts..." />
          </div>
        </div>
      </DashboardLayout>
    );
  ...
  return (
    <DashboardLayout title="Admin Dashboard" subtitle={`Welcome back, Admin ${user?.firstName || ''}!`}>
      {/* Statistics Section */}
        ...
        <div className="business-summary-section">
          ...
          <div className="summary-grid">
            <div className="summary-card">
              <div className="summary-icon" style={{ backgroundColor: '#15803d20' }}>
        ...
        <div className="chart-grid">
          ...
          <div className="chart-card chart-card-full">
            <div className="chart-header">
              ...
              <Warehouse size={18} color="var(--primary-green)" />
            </div>
      ...
      <div className="dashboard-section" style={{ border: '1px solid var(--border-light)', borderRadius: 'var(--radius-lg)', marginTop: '32px' }}>
        <div className="section-header">
          ...
          <div className="activity-controls">
            ...
            <div onClick={() => navigate('/transactions')} className="view-all-link" style={{ cursor: 'pointer' }}>
              View All
            </div>
          </div>
        </div>
        
        <div className="activity-list" style={{ border: 'none', boxShadow: 'none', padding: 0 }}>
          {filteredActivity.length === 0 ? (
            <div className="no-activity">
              <Activity size={32} />
              <p>No recent activity matching your search</p>
            </div>
          ) : (
                  ...
                  {activity.type === 'Transaction' ? (
                    <>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                        <span style={{ fontSize: '11px', color: 'var(--text-light)' }}>Payment:</span>
                        <StatusBadge status={activity.status} />
                      </div>
                      {activity.deliveryStatus && (
                        <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                          <span style={{ fontSize: '11px', color: 'var(--text-light)' }}>Delivery:</span>
                          <StatusBadge status={activity.deliveryStatus} />
                        </div>
                      )}
                    </>
                  ) : (
