...
const EditWarehouse = () => {
  ...
  // Fetch all storekeepers for the dropdown - only STOREKEEPER users
  const { data: storekeepersData } = useQuery(
    'storekeepers-for-warehouse',
    async () => {
      // Use the endpoint that filters by userType to get only storekeepers
  ...
  const fetchWarehouse = async () => {
    try {
      ...
      // Fetch warehouse accesses to find current storekeeper
      try {
        ...
        // Continue without storekeeper info if fetch fails
      }
      
      // Ensure availableCapacityKg is set - use totalCapacityKg if not available
      ...
      if (warehouse.location) {
        ...
        if (location.type === 'Sector') {
          setSelectedSector(location.id);
          // Fetch parent district
          if (location.parent) {
            setSelectedDistrict(location.parent.id);
            // Fetch parent province
            if (location.parent.parent) {
              setSelectedProvince(location.parent.parent.id);
              // Load districts for this province
              dataService.getChildLocations(location.parent.parent.id)
                .then((response) => setDistricts(response.data || []))
                .catch(() => {});
              // Load sectors for this district
              dataService.getChildLocations(location.parent.id)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Update storekeeper assignment if changed
      ...
      if (newStorekeeperId !== currentId) {
        if (newStorekeeperId) {
          ...
          // Storekeeper was removed - deactivate the warehouse access
          try {
            // Find and deactivate the current storekeeper's access
    ...
    <div className="form-page">
      ...
      <div className="form-container">
        <div className="form-card">
          ...
          <p className="form-subtitle">Update warehouse information</p>
